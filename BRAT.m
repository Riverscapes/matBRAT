...
for k = 1:(length(data))
    ...
    if e_DamCt (k) ~= isNaN 
       e_DamDens (k) = e_DamCt (k)/((iGeo_Length(k))/1000);
       e_DamPcC (k) = e_DamCT(k)/oCC_EX(k); 
    end
